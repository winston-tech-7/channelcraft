...
router.post("/", async (req, res) => {
  try {
    ...
    const images = await Promise.all(
      [1, 2, 3].map(() => generateChannelCover(composedPrompt, parsed.template, "standard"))
    );

    const urls: string[] = [];
    for (const [index, image] of images.entries()) {
      const watermarkSvg = Buffer.from(
        `<svg width="1280" height="640"><text x="24" y="620" fill="white" opacity="0.75" font-size="28">ChannelCraft Free</text></svg>`
      );
