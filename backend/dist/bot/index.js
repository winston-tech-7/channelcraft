...
bot.callbackQuery(/buy_hd:(\d+)/, async (ctx) => {
    ...
    if (!ctx.chat)
        return;
    await ctx.api.sendInvoice(ctx.chat.id, "HD Cover Export", "Unwatermarked 1280x640 HD cover", createHdPayload(id), "XTR", [{ label: "HD version", amount: PAYMENT_PRICES.hdSingle }]);
});
bot.command("pro", async (ctx) => {
    if (!ctx.chat)
        return;
    await ctx.api.sendInvoice(ctx.chat.id, "ChannelCraft Pro", "Unlimited generations and exclusive templates", createProPayload(), "XTR", [{ label: "Pro monthly", amount: PAYMENT_PRICES.proMonthly }]);
});
